...
// 레이어간 TraceId를 파라미터로 전달하지 않아도 된다.
// 여러번 호출 시 동시성 문제가 발생한다.
// FieldLogTrace는 싱글톤으로 스프링 컨테이너에 등록 되므로, traceIdHolder를 여러 쓰레드가 동시 접근하면 문제가 발생한다.
// 동시성 문제는 지역 변수에서는 일어나지 않고, 인스턴스 필드나 static같은 공용 영역에서 발생한다.
