...
### 스프링 AOP 실무 주의사항
#### 내부 호출
- 스프링 AOP는 프록시에서 적용 되므로, 대상 객체 내부에서 메서드 호출이 발생하면 프록시를 거치지 않아 AOP를 호출하지 않음
  - 테스트 : `CallServiceV0Test.java`
- 대안1 - 자기 자신 주입
  - 테스트 : `CallServiceV1.java`
- 대안2 - 지연 조회
  - 테스트 : `CallServiceV2Test.java`
- 대안3 - 구조 변경
  - AOP는 주요 컴포넌트에만 적용한다.(public)
  - 테스트 : `CallServiceV3Test.java`

#### 타입 캐스팅 문제
- `ProxyFactory` 옵션 중,
  - `proxyTargetClass=false` -> JDK 동적 프록시 사용
  - `proxyTargetClass=true` -> CGLIB 사용(기본값)
- 인터페이스 기반으로 프록시를 생성하는 JDK 동적 프록시는 구체 클래스로 타입 캐스팅이 불가능
  - 테스트 : `ProxyCastingTest.java`
- 이는 의존관계 주입 시 문제를 발생시킬 수 있다.
  - ![JDK 동적 프록시](./images/image012.png)
    - 인터페이스로 프록시를 만들기 때문에 구현체로 캐스팅을 못하여 문제 발생
  - ![CGLIB](./images/image013.png)
    - 구체 클래스 기반으로 프록시가 생성되므로 인터페이스, 구체 클래스 모두 캐스팅이 가능

#### CGLIB의 단점
- 대상 클래스에 기본 생성자 필수
  - CGLIB 프록시는 대상 클래스를 상속 받고, 생성자에서 대상 클래스의 기본 생성자를 호출하므로 대상 클래스에 기본 생성자르르 만들어야 한다.
- 생성자 2번 호출 문제
  - 1번째 : 실제 타겟 객체를 생성할 때
  - 2번째 : 프록시 객체를 생성할 때(부모의 생성자)
- final 키워드 클래스, 메서드 사용 불가

#### 스프링의 해결책
- CGLIB을 스프링 내부에 함께 패키징
  - 별도의 라이브러리 추가 X
- `objenesis`라는 라이브러리로 기본 생성자 없이 객체 생성 가능
  - 이 라이브러리는 생성자 호출 없이 객체를 생성할 수 있게 해준다.
- `objenesis`로 생성자 2번 호출 문제 해결(1번만 호출 됨)
- CGLIB 기본 사용
  - 부트 2.0부터 CGLIB를 기본으로 사용한다.
