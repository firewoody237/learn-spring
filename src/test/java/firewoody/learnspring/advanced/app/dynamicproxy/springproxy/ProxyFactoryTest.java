...
import firewoody.learnspring.advanced.app.dynamicproxy.cglib.ConcreteService;
...
public class ProxyFactoryTest {
    ...
    void interfaceProxy() {
        ...
        ProxyFactory proxyFactory = new ProxyFactory(target); // ProxyFactory에서 생성할 프록시를 결정
    ...
    void proxyTargetClass() {
        ServiceInterface target = new ServiceImpl();
        ProxyFactory proxyFactory = new ProxyFactory(target); // ProxyFactory에서 생성할 프록시를 결정
        proxyFactory.setProxyTargetClass(true); // true이면 항상 CGLIB (기본값 = true, 따라서 인터페이스여도 항상 CGLIB 생성)
        proxyFactory.addAdvice(new TimeAdvice());
